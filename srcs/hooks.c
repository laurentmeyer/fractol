...
/*   Updated: 2016/12/13 14:07:42 by lmeyer           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//		newton_update_all(data);
//	}
	if (data && keycode == ESC_KEY)
		exit(0);
	return (1);
}
...
//static int				mouse_click_drag(int x, int y, t_data *data)
//{
...
int				all_hooks(t_data *data)
{
	...
	if (data->julia)
	{
		mlx_key_hook(data->julia->win_ptr, &key_hooks, d);
	}
	//mlx_hook(data->win, MotionNotify, Button1MotionMask, &mouse_click_drag, d);
